...
export const PROCESSOR_CATALOG: ProcessorDef[] = [
  ...
  },
  {
    ...
    label: "Leer SQL Server",
    category: "source",
    description: "Lee SQL Server por lotes (Tiberius) y emite cada fila como objeto JSON.",
    fields: [
      {
        key: "connection",
        label: "Conexión",
        ...
        required: true,
        help: "Alias de un perfil SQL Server en Conexiones.",
      },
      {
        key: "query",
        label: "Consulta SQL",
        kind: "textarea",
        required: true,
        placeholder: "SELECT TOP (100) [id], [name] FROM [dbo].[tabla] WHERE [active] = @P1",
        help: "Solo SELECT/WITH. Placeholders `@P1`, `@P2`, …. Puedes crearla desde Conexiones → Constructor SQL.",
      },
      { key: "batch_size", label: "Tamaño de lote", kind: "number" },
    ],
    defaultConfig: { connection: "", query: "", batch_size: 1000 },
  ...
  },
  {
    ...
    label: "JME upsert",
    category: "transform",
    description: "Escribe estado de dominio en el Jaiba Memory Engine (requiere engine.domain_memory).",
    fields: [
      { key: "class", label: "Clase JME", kind: "text", required: true, placeholder: "carrier" },
      { key: "id_attribute", label: "Atributo ID", kind: "text", required: true, placeholder: "carrier.id" },
      { key: "value_attribute", label: "Atributo valor (JSON)", kind: "text", placeholder: "carrier.json" },
    ],
    defaultConfig: { class: "", id_attribute: "", value_attribute: "" },
  },
  {
    type: "memory_get",
    label: "JME get",
    category: "transform",
    description: "Lee estado de dominio del JME hacia un atributo del paquete.",
    fields: [
      { key: "class", label: "Clase JME", kind: "text", required: true, placeholder: "carrier" },
      { key: "id_attribute", label: "Atributo ID", kind: "text", required: true, placeholder: "carrier.id" },
      { key: "attribute", label: "Atributo destino", kind: "text", placeholder: "memory.value" },
      { key: "miss_as_null", label: "Miss como null", kind: "boolean" },
    ],
    defaultConfig: { class: "", id_attribute: "", attribute: "memory.value", miss_as_null: false },
  },
  {
    type: "memory_remove",
    label: "JME remove",
    category: "transform",
    description: "Elimina una clave del Jaiba Memory Engine.",
    fields: [
      { key: "class", label: "Clase JME", kind: "text", required: true, placeholder: "carrier" },
      { key: "id_attribute", label: "Atributo ID", kind: "text", required: true, placeholder: "carrier.id" },
    ],
    defaultConfig: { class: "", id_attribute: "" },
  },
  {
    type: "log_records",
    label: "Registrar (log)",
